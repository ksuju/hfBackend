...
public class ApiV1CommentController {
    ...
    @GetMapping("/comments/{festival-id}")
    ...
    public List<CommentDto> getReplies(@PathVariable("super-comment-id") String superCommentId) {
        return commentService.searchBySuperCommentId(superCommentId);
    }

    // 해당 게시글에 댓글 생성
    @PostMapping("/comments/{festival-id}")
    ...
    public ResponseEntity<String> updateComment(@PathVariable("comment-id") String commentId, @RequestBody @Valid UpdateCommentForm updateCommentForm, @LoginUser Member loginUser) {
    ...
    public ResponseEntity<String> deleteComment(@PathVariable("comment-id") String commentId, @LoginUser Member loginUser) {
